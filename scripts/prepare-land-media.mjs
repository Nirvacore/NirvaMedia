...
try {
  ...
  const templates = content.templateDrafts ? `\n\n## Apology wording drafts — not narration or approved translations\n\n${content.templateDrafts.map(item => `### ${item.name} (${item.locale})\n\nStatus: ${item.status}\n\n${item.text}`).join('\n\n')}\n` : '';
