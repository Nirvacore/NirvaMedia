...
import { applyMiddleware } from "./server/middleware/index.ts";
import { initWebSocket } from "./server/ws/index.ts";
...
function vitePluginAnalytics(): Plugin {
  return {
    name: "nirva-analytics",
    transformIndexHtml(html) {
      const endpoint = process.env.VITE_ANALYTICS_ENDPOINT;
      const websiteId = process.env.VITE_ANALYTICS_WEBSITE_ID;
      if (!endpoint || !websiteId || endpoint.includes("%")) return html;
      return html.replace(
        "</body>",
        `    <script defer src="${endpoint.replace(/\/$/, "")}/umami" data-website-id="${websiteId}"></script>\n  </body>`
      );
    },
  };
}
